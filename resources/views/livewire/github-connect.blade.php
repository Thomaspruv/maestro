<div class="space-y-3">
    ...
    @elseif($authMode === 'oauth')
        <a
            href="{{ $connectUrl }}"
            class="inline-flex w-full items-center justify-center gap-2.5 rounded-lg border border-[#30363d] bg-[#24292f] px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-[#32383f] {{ $this->compact ? 'py-2 text-xs' : '' }}"
        >
    ...
    @else
        ...
        <div class="rounded-lg border border-bg-overlay bg-bg-surface/40 px-4 py-3 space-y-3">
            <x-maestro.input
                ...
                label="{{ $this->compact ? 'Token GitHub' : 'Étape 2 — coller le token ici' }}"
                type="password"
                placeholder="ghp_..."
